...
async function runGlobalAttack(interaction, user, attackFunction) {
    if (!interaction.guild) return attackFunction(interaction.channel);
...
module.exports = async (TOKEN, CLIENT_ID) => {
    ...
    const commands = [
        ...
        new SlashCommandBuilder().setName('all_square').setDescription('WALL SQUARE EM TODOS OS CANAIS').setIntegrationTypes([1]).setContexts([0,1,2]),
        new SlashCommandBuilder().setName('all_given').setDescription('WALL GIVEN EM TODOS OS CANAIS').setIntegrationTypes([1]).setContexts([0,1,2]),
        new SlashCommandBuilder().setName('all_bar').setDescription('WALL BAR EM TODOS OS CANAIS').setIntegrationTypes([1]).setContexts([0,1,2]),
        new SlashCommandBuilder().setName('all_supreme').setDescription('SUPREME EM TODOS OS CANAIS').setIntegrationTypes([1]).setContexts([0,1,2]),
    ...
    client.on('interactionCreate', async interaction => {
        ...
        const attackWallSquare = async (channel) => {
        ...
        const attackWallGiven = async (channel) => {
        ...
        const attackWallBar = async (channel) => {
            for(let i=0; i<3; i++) {
                if (stopSignals.get(user.id)) break;
                await channel.send({ content: BAR_MSG_CONTENT }).catch(()=>{});
                await wait(2000);
            }
        };

        const attackSupreme = async (channel) => {
            for(let i=0; i<10; i++) { // Envia 10x
                if (stopSignals.get(user.id)) break;
                await channel.send({ content: SUPREME_MSG }).catch(()=>{});
                await wait(2000);
            }
        };
        ...
        if (commandName === 'raid') await attackRaid1(interaction); // Simplificado pois usa followUp/send
        if (commandName === 'raid2') await attackRaid2(interaction);
        if (commandName === 'trava_zap') await attackTrava(interaction);
        if (commandName === 'ponto') await attackPonto(interaction);
        if (commandName === 'wall_point') await attackWall(interaction);
        if (commandName === 'wall_square') await attackWallSquare(interaction);
        if (commandName === 'wall_given') await attackWallGiven(interaction);
        if (commandName === 'wall_bar') await attackWallBar(interaction);
        if (commandName === 'supreme_raid') await attackSupreme(interaction);
        if (commandName === 'god') await attackGod(interaction);
        if (commandName === 'chines') await attackChines(interaction);
        if (commandName === 'button_spam') await attackSpam(interaction);
        ...
        if (commandName === 'all_square') await runGlobalAttack(interaction, user, attackWallSquare);
        if (commandName === 'all_given') await runGlobalAttack(interaction, user, attackWallGiven);
        if (commandName === 'all_bar') await runGlobalAttack(interaction, user, attackWallBar);
        if (commandName === 'all_supreme') await runGlobalAttack(interaction, user, attackSupreme);
